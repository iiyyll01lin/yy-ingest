...
from .utils import rigorous_document_search, get_openai_embedding_function
...
class BaseChunkerRunner:
    def __init__(
        self,
        questions_csv_path=None,
        chroma_db_path=None,
        corpora_id_paths=None,
        markdown_dir="chunker/test_data/",
        original_pdf_name="",
    ):
        self.corpora_id_paths = corpora_id_paths
        self.markdown_dir = markdown_dir
        # self.markdown_dir_path = "/data/zqy/MinerU/result/"
        self.questions_csv_path = questions_csv_path
        self.original_pdf_name = original_pdf_name

        self.corpus_list = []

        self._load_input_markdowns()
    ...
    def _load_input_markdowns(self):
        if os.path.exists(self.markdown_dir):
            ...
            for root, dirs, files in os.walk(self.markdown_dir):
                for file in files:
                    if file.endswith(".md"):
                        # Get full path of the markdown file
                        full_path = os.path.join(root, file)
                        md_files.append(full_path)
                        print(f"[_load_input_markdowns] {full_path} load done!")

            # Store the paths in corpus_list
            self.corpus_list = md_files
        else:
            print("[_load_input_markdowns] markdown_dir not found")
            self.corpus_list = []

    # TODO: prepare for chunker evaluation
    ...
    def _get_chunks_and_metadata(self, splitter):
        ...
        for corpus_id in self.corpus_list:
            ...
            # input of the raw markdown corpus
            ...
            for document in current_documents:
                ...
                # print(f"[_get_chunks_and_metadata] {corpus_id} split done!")
        ...
        for index, row in self.questions_df.iterrows():
            ...
            for metadata in chunk_metadatas:
                ...
                for ref_obj in references:
                    ...
                    ref_start, ref_end = int(ref_obj["start_index"]), int(
                        ref_obj["end_index"]
                    )
    ...
    def run(self, chunker):
        # Get chunks and metadata from the chunker
        docs, metas = self._get_chunks_and_metadata(chunker)

        # Process and group chunks by corpus_id
        chunks_by_corpus = {}
        for meta in metas:
            corpus_id = meta["corpus_id"]
            if corpus_id not in chunks_by_corpus:
                chunks_by_corpus[corpus_id] = []
            chunks_by_corpus[corpus_id].append(meta)

        # Create a list of dictionaries combining chunks with their metadata
        json_chunks_with_metadata = []
        for doc, meta in zip(docs, metas):
            # Get the total chunks for this file (span)
            corpus_id = meta["corpus_id"]
            total_chunks = len(chunks_by_corpus[corpus_id])

            # Find the index of this chunk in its corpus (span_count)
            corpus_chunks = chunks_by_corpus[corpus_id]
            span_count = next(
                (
                    i + 1
                    for i, m in enumerate(corpus_chunks)
                    if m["start_index"] == meta["start_index"]
                    and m["end_index"] == meta["end_index"]
                ),
                0,
            )

            # Extract the original md file name
            md_file_name = os.path.basename(corpus_id)
            ...
            chunk_data = {
                ...
                "metadata": {
                    ...
                    # "corpus_id": meta["corpus_id"],
                    "total_span": total_chunks,
                    "span_count": span_count,
                    "original_file": self.original_pdf_name,
                },
            }
            json_chunks_with_metadata.append(chunk_data)

        return json_chunks_with_metadata
