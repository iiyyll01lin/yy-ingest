...
class ClusterSemanticChunker(BaseChunker):
    ...
    def split_text(self, text: str) -> List[str]:
        ...
        # fixing url space join
        # docs = [' '.join(sentences[start:end+1]) for start, end in clusters]

        # first apply all space join, then find img pattern to remove all the space within it
        docs = []
        for start, end in clusters:
            joined_text = " ".join(sentences[start : end + 1])
            ...
            def fix_img_tag(match):
                ...
                src_content = re.search(r'src="([^"]*)"', entire_tag).group(1)

                # Remove all spaces within the URL
                fixed_src = re.sub(r"\s+", "", src_content)

                # Replace the original src with the fixed one
                return entire_tag.replace(src_content, fixed_src)

            # Apply the fix to each img tag
            fixed_text = re.sub(img_pattern, fix_img_tag, joined_text)

            docs.append(fixed_text)
